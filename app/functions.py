...
def triage(data: dict):
    ...
    if n_ssl == 0 and n_adenoma < 3 and size_adenoma < 10 and hgd_adenoma == 'no':
    ...
    if outcome['follow_up'] + patient_age > 75:
...
async def main():
    with open(DATA_PATH / 'sample_patient_report_2.txt', 'r', encoding = 'utf-8') as f:
        report = f.read()
